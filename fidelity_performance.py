from numpy import expand_dims, gradient
...
class Prototype:
    ...
    def run(self, csv_files: list):
        ...
        for file in csv_files:
            df: DataFrame = pd.read_csv(file, index_col="Run Date", parse_dates=["Run Date"], infer_datetime_format=True)
            dfs.append(df)

        df_merged = reduce(lambda  left,right: left.append(right), dfs)
        df_merged =  df_merged.sort_index()

        if self.end_date:
            df_merged = df_merged[:self.end_date]
